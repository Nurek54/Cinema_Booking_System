...
function show(view){
    ...
    adminLoginSec.hidden  = view!=='admin-login';
...
document.getElementById('movie-form').onsubmit = async e=>{
    ...
    await fetch('/api/movies', {
        ...
        body: JSON.stringify({
            ...
            durationMinutes: +durInput.value
        })
    });
    e.target.reset();
    loadMovies();
...
async function loadScreenings(){
    ...
    // wypełnienie dropdown w rezerwacji
...
document.getElementById('screening-form').onsubmit = async e=>{
    ...
    await fetch('/api/screenings', {
        ...
        headers:{'Content-Type':'application/json'},
        body: JSON.stringify({
            movie: { id: +scMovieSel.value },
            room: { id: +scRoomSel.value },
            dateTime: scDateInp.value
        })
    });
    e.target.reset();
    loadScreenings();
...
btnBook.onclick = async () => {
    const r = await fetch('/api/reservations/' + curScreenId, {
        method: 'POST',
        headers: {'Content-Type':'application/json'},
        body: JSON.stringify([...selected])
    });
    resStatus.textContent = r.ok ? '✔ Zarezerwowano' : await r.text();
    if(r.ok){
        loadScreenings();
        setTimeout(()=>show('screens'), 700);
    }
};
...
async function loadAdminReservations(){
    ...
    adminResDiv.innerHTML = res.length
        ? `<table>
             <thead>
               <tr><th>ID</th><th>Seans</th><th>Miejsca</th><th></th></tr>
             </thead>
             <tbody>${
            res.map(r=>`
               <tr>
                 <td>${r.id}</td>
                 <td>${r.screening.id}</td>
                 <td>${r.seats.join(', ')}</td>
